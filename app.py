...
class App(customtkinter.CTkToplevel):
    ...
    def __init__(self, user, master):
        ...
        self.clean = CTkButton(master = self.left, width = 140, text = "Cleaner\t ", text_font = ("Roboto Medium", -16), fg_color = "#486ee0", hover_color = "#4063c9", image = ImageTk.PhotoImage(Image.open("assets/clean.png").resize((20, 20), Image.ANTIALIAS)), command = lambda: [self.setAllButtonsToNormal(), self.clean.configure(state = tkinter.DISABLED, fg_color = "#616160"), self.wm_title(f"Hyperium Optimizer v{self.version} | Clean"), self.clearRightFrame(), self.cleanPanel()])
        self.clean.grid(row = 4, column = 0, padx = 5, pady = 7)

        # -- -- Virus Scan

        self.virusScan = CTkButton(master = self.left, width = 140, text = "Virus Scan", text_font = ("Roboto Medium", -16), fg_color = "#486ee0", hover_color = "#4063c9", image = ImageTk.PhotoImage(Image.open("assets/virus.png").resize((20, 20), Image.ANTIALIAS)), command = lambda: [self.setAllButtonsToNormal(), self.virusScan.configure(state = tkinter.DISABLED, fg_color = "#616160"), self.wm_title(f"Hyperium Optimizer v{self.version} | Virus Scan"), self.clearRightFrame(), self.virusScanPanel()])
    ...
    def setAllButtonsToNormal(self):
        self.optimize.configure(state = tkinter.NORMAL, fg_color = "#486ee0")
